...
{/* 🎨 EXCALIDRAW DIAGRAM #1: Architecture Overview */}

<img src="/images/architecture-diagram.svg" alt="Architecture: Frontend → Vercel Edge Functions → ElevenLabs TTS API, with localStorage for credits and Dodo Payments for checkout" />
...
{/* 🎨 EXCALIDRAW DIAGRAM #2: Payment Flow */}

<img src="/images/payment-flow.svg" alt="Payment flow: User clicks Buy → Frontend calls /api/checkout → Dodo returns checkout link → User pays → Redirects back → Credits added" />
...
{/* 🎨 EXCALIDRAW DIAGRAM #3: Credit Deduction Flow */}

<img src="/images/credit-flow.svg" alt="Credit flow: Enter text → Enough credits? → Yes: generate + deduct + play. No: show pricing modal." />
