...
After checkout, Dodo fires a `payment.succeeded` webhook to your server. Your handler maps the purchased product to a character count and credits the customer's wallet. No URL params, no localStorage — the server is the source of truth.
...
The `idempotency_key` is critical — Dodo retries failed webhooks, and using `paymentId` ensures the wallet is only credited once per payment.

<Info>
  Register your webhook URL in the [Dodo Dashboard](https://dashboard.dodopayments.com) under **Webhooks**. Set the URL to `https://your-app.vercel.app/api/webhook` and filter for `payment.succeeded` events.
</Info>
