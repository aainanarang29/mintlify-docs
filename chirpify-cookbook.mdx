...
Your **frontend** talks to **Vercel serverless functions** that use the [Dodo Payments SDK](https://www.npmjs.com/package/dodopayments) to create customers with wallets, call ElevenLabs for audio generation (checking and debiting the wallet server-side), and create checkout sessions for purchases. When a payment succeeds, a **webhook** credits the customer's wallet automatically. No database needed Dodo Wallets are the source of truth.

<Frame>
  ![Credit Purchase Flow](/images/credit-purchase-flow.jpeg)
</Frame>

<Frame>
  ![Speech Generation Flow](/images/speech-generation-flow.jpeg)
</Frame>
...
| `POST /api/checkout` | Create checkout session linked to customer           |
...
The `dev` script uses Node's built-in `--env-file` flag to load your `.env` automatically. Requires **Node 20.6+** (run `node -v` to check).
...
1. **`customer: { customer_id: customerId }`** links the checkout session to an existing Dodo customer. This is what connects the payment to the right wallet.
2. **No characters in the return URL** — credits are added server-side via webhook, not client-side via URL params.
3. **`billing_address: { country: 'US' }`** — hardcoded for the demo. In production, collect the user's actual country so Dodo calculates the correct tax.

<Info>
  We use `dodo.checkoutSessions.create()` instead of the deprecated `payments.create()` endpoint. Checkout sessions always return a `checkout_url` — no extra flags needed.
</Info>
...
| `DODO_ENV`                  | SDK environment (`test_mode`/`live_mode`) | `test_mode`                      |
| `DODO_PAYMENTS_WEBHOOK_KEY` | Verifies webhook signatures from Dodo     | `whsec_...`                      |
...
1. **`Use checkoutSessions.create(), not payments.create()`** — the payments endpoint is deprecated. Checkout sessions always return a `checkout_url`.
