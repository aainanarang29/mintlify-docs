...
description: "Let's ship a TTS app with ElevenLabs and Dodo Payments, complete with a free tier, credit packs, and checkout."
---

# [Chirpify AI: Text-to-Speech with Per-Character Billing](https://chirpify-ai.vercel.app/)

By the end of this cookbook, you'll have a fully deployed text-to-speech app that lets users type text, pick a voice, generate speech, and pay for it — with a freemium model, three credit tiers, server-side wallet tracking, and a real checkout flow powered by webhooks.

<video src="https://mintcdn.com/aainanarang2911gmailcom/cdsleP44WXl152uN/videos/Astute_Platinum_Saga-1.mp4?fit=max&auto=format&n=cdsleP44WXl152uN&q=85&s=694f35c3eda7ba6c6d71e42c537b7681" controls={true} data-path="videos/Astute_Platinum_Saga-1.mp4" />
...
<Frame>
  ![Whats App Image2026 02 28at12 06 56AM](/images/WhatsAppImage2026-02-28at12.06.56AM.jpeg)
</Frame>
...
<Warning>
  Never commit `.env` to version control. Create a `.gitignore` before your first commit.
</Warning>
...
<Note>
  The signature is computed over the raw request body. Vercel's default body parser delivers `req.body` as a parsed object, so the code re-serializes it with `JSON.stringify`. This works for Dodo payloads, but for bulletproof verification in high-stakes apps, use a [custom body parser](https://vercel.com/docs/functions/runtimes#advanced-usage) to capture the raw bytes and sign against those instead.
</Note>

<Info>
  Register your webhook URL in the [Dodo Dashboard](https://dashboard.dodopayments.com) under **Webhooks**. Set the URL to `https://your-app.vercel.app/api/webhook` and filter for `payment.succeeded` events. Copy the **signing secret** (starts with `whsec_`) and add it as `DODO_WEBHOOK_SECRET` in your environment variables.
</Info>
...
In the [Dodo Dashboard](https://dashboard.dodopayments.com), go to **Webhooks** and create one:
...
<CardGroup cols={2}>
  ...
  <Card icon="shield-check" title="Check → Generate → Debit">
